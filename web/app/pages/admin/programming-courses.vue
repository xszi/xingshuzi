...
const columns = [
  ...
  { key: 'title', label: '标题', minWidth: 150 },
...
const loadData = async () => {
  loading.value = true
  try {
    const response = await api.get<any>('/courses/admin/list?category=programming')
    // 兼容不同的响应格式
...
const handleSubmit = async (data: Record<string, any>) => {
  try {
    ...
    if (isEditing.value) {
      await api.put(`/courses/admin/${currentItem.value.id || currentItem.value._id}`, submitData)
      ElMessage.success('更新成功')
    } else {
      await api.post('/courses/admin', submitData)
...
const handleDelete = async (item: any) => {
  ElMessageBox.confirm(
    ...
    try {
      await api.delete(`/courses/admin/${item.id || item._id}`)
